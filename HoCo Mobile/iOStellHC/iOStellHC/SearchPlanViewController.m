...
#import "SDPAnnotation.h"
#import "SIPAnnotation.h"
#import "PresubmissionAnnotation.h"
#import "WaiverAnnotation.h"
#import "PublicHearingAnnotation.h"
#import "TileOverlay.h"
#import "TileOverlayView.h"
#import "Annotation.h"
#import <QuartzCore/QuartzCore.h>

@interface SearchPlanViewController () {
...
@implementation SearchPlanViewController

@synthesize latItem,lngItem,locationManager,searchPlanTableView,mapView,mapInfoSegment,mapTypeSegment,historySwitch,myToolbar,historyToolbar,scrollLabel;
...
{
    [super viewDidLoad];
    self.searchPlanTableView.delegate = self;
    self.searchPlanTableView.dataSource = self;
    ...
    UIBarButtonItem *searchButton = [[UIBarButtonItem alloc]initWithBarButtonSystemItem:UIBarButtonSystemItemSearch target:self action:@selector(showMenu)];
    ...
    */
    ...
    alertViewer = [[CustomAlertView alloc] initWithTitle:@"What's next" message:@"Tap ‘Search’ button to search from your current location (2 mile radius).  Touch and drag on map to change the search location." delegate:nil cancelButtonTitle:@"Close" otherButtonTitles:nil];
    
    [alertViewer setDelegate:self];
    [alertViewer show];
    
    searchType = @"SiteDevelopmentPlans";
...
- (void)mapView:(MKMapView *)mapView1 regionDidChangeAnimated:(BOOL)animated {
    ...
    latItem = centerCoordinate.latitude;
    lngItem = centerCoordinate.longitude;
...
-(void) searchPlan:(NSString *)sType {
    
    mapInfoSegment.hidden = NO;
    NSString *str;
    
    if (historySwitch.on) {
        str = [NSString stringWithFormat:@"http://data.howardcountymd.gov/iOSsearchPlans/getHistoryList.asp?sType=%@&cenY=%f&cenX=%f", sType,latItem,lngItem];
    } else {
        str = [NSString stringWithFormat:@"http://data.howardcountymd.gov/iOSsearchPlans/getCurrentList.asp?sType=%@&cenY=%f&cenX=%f", sType,latItem,lngItem];
    }
    ...
    if (json.count==0) {
         alertViewer = [[CustomAlertView alloc] initWithTitle:@"Not Found" message:@"No Plans in 2 miles radius of map center, please make sure that you are in Howard County Maryland" delegate:nil cancelButtonTitle:@"Close" otherButtonTitles:nil];
        [alertViewer setDelegate:self];
        [alertViewer show];
        
        [searchPlanTableView reloadData];
    } else {
        if ([sType isEqualToString:@"SiteDevelopmentPlans"]) {
            self.navigationItem.title = @"Site Development Plans";
        } else if ([sType isEqualToString:@"SubdivisionPlans"]) {
            self.navigationItem.title = @"Subdivision Plans";
        } else if ([sType isEqualToString:@"PresubmissionPlans"]) {
            self.navigationItem.title = @"Presubmission Community Meetings";
        } else if ([sType isEqualToString:@"PublicHearing"]) {
            self.navigationItem.title = @"Public Hearing";
        } else if ([sType isEqualToString:@"WaiverPetitions"]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.imageView.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@.png", searchType]];
    cell.textLabel.text = [info objectForKey:@"fName"];
    cell.detailTextLabel.text = [info objectForKey:@"fNumber"];
    return cell;
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    SearchPlanDetailViewController *detailVC = [[SearchPlanDetailViewController alloc]init];
    NSDictionary *info = [json objectAtIndex:indexPath.row];
    detailVC.idItem = [info objectForKey:@"myID"];
    detailVC.titleMap = [info objectForKey:@"fName"];
    detailVC.subTitleMap = [info objectForKey:@"fNumber"];
    detailVC.searchTypeItem = searchType;
    detailVC.latItem = [[info objectForKey:@"Y"]doubleValue];
    detailVC.lngItem = [[info objectForKey:@"X"]doubleValue];
...
- (void)setPin {
    ...
    circle = [MKCircle circleWithCenterCoordinate:centerCoordinate radius:3218.688];
    ...
    for (int i = 0; i < [json count]; i++) {
        if ([searchType isEqualToString:@"SiteDevelopmentPlans"]) {
            ...
            SDPAnnotation *ann = [[SDPAnnotation alloc] init];
            ann.title = [loc objectForKey:@"fName"];
            ann.subtitle = [loc objectForKey:@"fNumber"];
            ann.coordinate = location;
            ann.latItem = [[loc objectForKey:@"Y"]doubleValue];
            ann.lngItem = [[loc objectForKey:@"X"]doubleValue];
            ann.idItem = [loc objectForKey:@"myID"];
            [mapView addAnnotation:ann];
        } else if ([searchType isEqualToString:@"SubdivisionPlans"]) {
            NSDictionary *loc = [json objectAtIndex:i];
            location.latitude = [[loc objectForKey:@"Y"]doubleValue];
            location.longitude = [[loc objectForKey:@"X"]doubleValue];
            SIPAnnotation *ann = [[SIPAnnotation alloc] init];
            ann.title = [loc objectForKey:@"fName"];
            ann.subtitle = [loc objectForKey:@"fNumber"];
            ann.coordinate = location;
            ann.latItem = [[loc objectForKey:@"Y"]doubleValue];
            ...
            ann.idItem = [loc objectForKey:@"myID"];
            [mapView addAnnotation:ann];
        } else if ([searchType isEqualToString:@"PresubmissionPlans"]) {
            NSDictionary *loc = [json objectAtIndex:i];
            location.latitude = [[loc objectForKey:@"Y"]doubleValue];
            location.longitude = [[loc objectForKey:@"X"]doubleValue];
            PresubmissionAnnotation *ann = [[PresubmissionAnnotation alloc] init];
            ann.title = [loc objectForKey:@"fName"];
            ann.subtitle = [loc objectForKey:@"fNumber"];
            ann.coordinate = location;
            ann.latItem = [[loc objectForKey:@"Y"]doubleValue];
            ...
            ann.idItem = [loc objectForKey:@"myID"];
            [mapView addAnnotation:ann];
        } else if ([searchType isEqualToString:@"PublicHearing"]) {
            NSDictionary *loc = [json objectAtIndex:i];
            location.latitude = [[loc objectForKey:@"Y"]doubleValue];
            location.longitude = [[loc objectForKey:@"X"]doubleValue];
            PublicHearingAnnotation *ann = [[PublicHearingAnnotation alloc] init];
            ann.title = [loc objectForKey:@"fName"];
            ann.subtitle = [loc objectForKey:@"fNumber"];
            ann.coordinate = location;
            ann.latItem = [[loc objectForKey:@"Y"]doubleValue];
            ...
            ann.idItem = [loc objectForKey:@"myID"];
            [mapView addAnnotation:ann];
        } else if ([searchType isEqualToString:@"WaiverPetitions"]) {
            NSDictionary *loc = [json objectAtIndex:i];
            location.latitude = [[loc objectForKey:@"Y"]doubleValue];
            location.longitude = [[loc objectForKey:@"X"]doubleValue];
            WaiverAnnotation *ann = [[WaiverAnnotation alloc] init];
            ann.title = [loc objectForKey:@"fName"];
            ann.subtitle = [loc objectForKey:@"fNumber"];
            ann.coordinate = location;
            ann.latItem = [[loc objectForKey:@"Y"]doubleValue];
...
-(MKAnnotationView *)mapView:(MKMapView *)mView viewForAnnotation:(id <MKAnnotation>)annotation
{
    ...
    if ([annotation isKindOfClass:[SDPAnnotation class]]) {
        ...
        {            
            ...
            imgname = @"SiteDevelopmentPlans.png";
            ...
            CGSize maxSize = CGRectInset(self.view.bounds,
                                         [SearchPlanViewController annotationPadding],
                                         [SearchPlanViewController annotationPadding]).size;
            maxSize.height -= self.navigationController.navigationBar.frame.size.height + [SearchPlanViewController calloutHeight];
    ...
    else if ([annotation isKindOfClass:[SIPAnnotation class]]) {
        // try to dequeue an existing pin view first
        static NSString *annotationIdentifier1 = @"AnnotationIdentifier1";
        MKAnnotationView *pinView = (MKAnnotationView *) [mapView dequeueReusableAnnotationViewWithIdentifier:annotationIdentifier1];
        if (!pinView)
        {
    ...
    else if ([annotation isKindOfClass:[PresubmissionAnnotation class]]) {
        // try to dequeue an existing pin view first
        static NSString *annotationIdentifier2 = @"AnnotationIdentifier2";
        MKAnnotationView *pinView = (MKAnnotationView *) [mapView dequeueReusableAnnotationViewWithIdentifier:annotationIdentifier2];
        if (!pinView)
        {
            ...
            imgname = @"PresubmissionPlans.png";
            ...
            UIImageView *iconView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:imgname]];
            annotationView2.leftCalloutAccessoryView = iconView;
    ...
    else if ([annotation isKindOfClass:[PublicHearingAnnotation class]]) {
        // try to dequeue an existing pin view first
        static NSString *annotationIdentifier3 = @"AnnotationIdentifier3";
        MKAnnotationView *pinView = (MKAnnotationView *) [mapView dequeueReusableAnnotationViewWithIdentifier:annotationIdentifier3];
        if (!pinView)
        {
    ...
    else if ([annotation isKindOfClass:[WaiverAnnotation class]]) {
        // try to dequeue an existing pin view first
        static NSString *annotationIdentifier4 = @"AnnotationIdentifier4";
        MKAnnotationView *pinView = (MKAnnotationView *) [mapView dequeueReusableAnnotationViewWithIdentifier:annotationIdentifier4];
        if (!pinView)
        {
            ...
            UIImageView *iconView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:imgname]];
            annotationView4.leftCalloutAccessoryView = iconView;
...
- (void)mapView:(MKMapView *)mv annotationView:(MKAnnotationView *)pin calloutAccessoryControlTapped:(UIControl *)control {
    ...
    if ([control isKindOfClass:[UIButton class]]) {
        ...
        SearchPlanDetailViewController *detailVC = [[SearchPlanDetailViewController alloc]init];
        detailVC.idItem = theAnnotation.idItem;
        detailVC.titleMap = theAnnotation.title;
        detailVC.subTitleMap = theAnnotation.subtitle;
        detailVC.searchTypeItem = searchType;
        detailVC.latItem = theAnnotation.latItem;
        detailVC.lngItem = theAnnotation.lngItem;
...
- (IBAction)historySwitch:(id)sender {
    [self searchPlan:searchType];
}

- (void)showMenu {
    UIActionSheet *actionSht = [[UIActionSheet alloc]initWithTitle:nil delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"Presub. Community Meetings",@"Public Hearing",@"Site Development Plans",@"Subdivision Plans",@"Waiver Petitions",nil];
    [actionSht showFromToolbar:(UIToolbar *)myToolbar];
}

-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    NSString *buttonTitle = [actionSheet buttonTitleAtIndex:buttonIndex];
    historyToolbar.hidden = NO;
    searchPlanTableView.hidden = NO;
    if ([buttonTitle isEqualToString:@"Site Development Plans"]) {
        searchType = @"SiteDevelopmentPlans";
        [self searchPlan:searchType];
    } else if ([buttonTitle isEqualToString:@"Subdivision Plans"]) {
        searchType = @"SubdivisionPlans";
        [self searchPlan:searchType];
    }else if ([buttonTitle isEqualToString:@"Presub. Community Meetings"]) {
        searchType = @"PresubmissionPlans";
        [self searchPlan:searchType];
    } else if ([buttonTitle isEqualToString:@"Public Hearing"]) {
        searchType = @"PublicHearing";
        [self searchPlan:searchType];
    } else if ([buttonTitle isEqualToString:@"Waiver Petitions"]) {
        searchType = @"WaiverPetitions";
        [self searchPlan:searchType];
    }
    [self getMessages];
}
