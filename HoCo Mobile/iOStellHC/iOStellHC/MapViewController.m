...
@synthesize latItem,lngItem,randomItem,titleMap,address1Map,mapView,displayAll,mapEdit,neLong,neLat,swLong,swLat,locationManager,geoCoder;
...
-(IBAction)getLocation {
    ...
    neLong = neCoord.longitude;
    neLat = neCoord.latitude;
    
    CLLocationCoordinate2D swCoord;
    swCoord = [mapView convertPoint:swPoint toCoordinateFromView:mapView];
    swLong = swCoord.longitude;
...
- (void)viewDidLoad
{
    ...
    if (mapEdit==YES) {
        CustomAlertView *alertMap = [[CustomAlertView alloc] initWithTitle:@"Update Location" message:[NSString stringWithFormat:@"Drag pin on the map to update location"] delegate:nil cancelButtonTitle:@"Close" otherButtonTitles:nil];
        [alertMap show];
    ...
    if (displayAll==NO) {
        ...
        ann.subtitle = self.address1Map;
        ann.coordinate = region.center;
        [mapView addAnnotation:ann];
    }
    else if (displayAll==YES) {
        
        [self.navigationItem setTitle:@"Map All Issues"];
        ...
        for (int i = 0; i < [json count]; i++) {
            ...
            region.center.latitude = [[loc objectForKey:@"Y"]doubleValue];
            region.center.longitude = [[loc objectForKey:@"X"]doubleValue];
            Annotation *ann = [[Annotation alloc] init];
        ...
        region.center.latitude = 39.23629734128874;
        region.center.longitude = -76.941731781689;
...
- (void)coordinateChanged_:(NSNotification *)notification {
    ...
    [self.geoCoder reverseGeocodeLocation:myLocation completionHandler:^(NSArray *placemarks, NSError *error) {
        ...
        annotation.subtitle = [NSString	stringWithFormat:@"%@", locatedAt];
...
- (void)mapView:(MKMapView *)mapView annotationView:(MKAnnotationView *)annotationView didChangeDragState:(MKAnnotationViewDragState)newState fromOldState:(MKAnnotationViewDragState)oldState {
	...
	if (oldState == MKAnnotationViewDragStateDragging) {
        ...
        NSMutableString *postString = [NSMutableString stringWithString:@"http://data.howardcountymd.gov/iOStellHC/UpdateLocation.asp"];
        [postString appendString:[NSString stringWithFormat:@"?cenX=%f", annotation.coordinate.longitude]];
        [postString appendString:[NSString stringWithFormat:@"&cenY=%f", annotation.coordinate.latitude]];
        [postString appendString:[NSString stringWithFormat:@"&timestamp=%@", randomItem]];
