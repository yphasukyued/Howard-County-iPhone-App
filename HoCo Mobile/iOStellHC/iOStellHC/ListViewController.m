...
@synthesize latItem,lngItem,emailItem,listTableView,mapView,mapInfoSegment,mapTypeSegment,locationManager,scrollLabel;
...
- (void)viewDidLoad
{
    ...
    [self.navigationItem setTitle:@"tell HoCo"];
    
    UIBarButtonItem *backButton = [[UIBarButtonItem alloc]initWithTitle:@"List" style:UIBarButtonItemStylePlain target:nil action:nil];
...
- (void)showMenu {
    ...
    alertViewer = [[CustomAlertView alloc] initWithTitle:nil message:nil delegate:nil cancelButtonTitle:@"Close" otherButtonTitles:@"All Issues",@"My Issues",nil];
...
-(void)alertView:(CustomAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if ([buttonTitle isEqualToString:@"All Issues"]) {
        [self.navigationItem setTitle:@"All Issues"];
        listType = @"all";
        [self start];
    } else if ([buttonTitle isEqualToString:@"My Issues"]) {
        [self.navigationItem setTitle:@"My Issues"];
        listType = @"email";
        [self start];
    }
}
...
-(void)start {
    ...
    if ([listType isEqualToString:@"all"]) {
        NSString *str1 = @"http://data.howardcountymd.gov/iOStellHC/getListByAll.asp";
        NSString *str2 = [NSString stringWithFormat:@"%@?email=%@", str1,emailItem];
        NSURL *url = [NSURL URLWithString:str2];
        NSData *data = [NSData dataWithContentsOfURL:url];
        [self getData:data];
    } else if ([listType isEqualToString:@"email"]) {
        NSString *str1 = @"http://data.howardcountymd.gov/iOStellHC/getListByEmail.asp";
        NSString *str2 = [NSString stringWithFormat:@"%@?email=%@", str1,emailItem];
        NSURL *url = [NSURL URLWithString:str2];
        NSData *data = [NSData dataWithContentsOfURL:url];
        [self getData:data];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    thumbImageData = [NSData dataFromBase64String:cleanedStr];
    
    UIImage *newImage = [UIImage imageWithData:thumbImageData];
    
    
    CGSize itemSize = CGSizeMake(80,80); // give any size you want to give
    ...
    //scaledImage = [newImage resizedImageWithContentMode:UIViewContentModeScaleAspectFill bounds:CGSizeMake(80.0f, 80.0f) interpolationQuality:kCGInterpolationHigh];
    
    //NSLog(@"Image Size: %fx%f", scaledImage.size.height,scaledImage.size.width);
    
    cell.textLabel.text = [info objectForKey:@"Category"];
    cell.imageView.image = newImage;
    cell.detailTextLabel.text = [info objectForKey:@"Incident"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSDictionary *info = [json objectAtIndex:indexPath.row];
    detailVC.idItem = [info objectForKey:@"ID"];
...
- (void)setPin {
    ...
    for (int i = 0; i < [json count]; i++) {
        ...
        ann.title = [loc objectForKey:@"Category"];
        ann.subtitle = [loc objectForKey:@"Incident"];
        ann.status = [loc objectForKey:@"Status"];
        ann.coordinate = location;
        ann.idItem = [loc objectForKey:@"ID"];
...
- (void)mapView:(MKMapView *)mapView didAddAnnotationViews:(NSArray *)views {
    NSLog(@"test1");
    MKPinAnnotationView *annotationView = [views objectAtIndex:0];
...
-(MKAnnotationView *)mapView:(MKMapView *)mView viewForAnnotation:(id <MKAnnotation>)annotation
{
    ...
    if ([annotation isKindOfClass:[Annotation class]])
    {
        ...
        if ([ann.status isEqualToString:@"Acknowledged"]) {
            annotationView.pinColor = MKPinAnnotationColorRed;
        } else if ([ann.status isEqualToString:@"Work In Progress"]) {
            annotationView.pinColor = MKPinAnnotationColorPurple;
        } else if ([ann.status isEqualToString:@"Completed"]) {
