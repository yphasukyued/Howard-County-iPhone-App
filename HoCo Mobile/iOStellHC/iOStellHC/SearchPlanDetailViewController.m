//
//  SearchPlanDetailViewController.m
//  iOStellHC
//
...
#import "SearchPlanDetailViewController.h"
#import "Annotation.h"
#import "TileOverlay.h"
#import "TileOverlayView.h"
#import "WebViewController.h"

@interface SearchPlanDetailViewController () {
    NSMutableArray *json;
    NSString *imgname;
    NSString *pMail;
    NSString *eMail;
...
@implementation SearchPlanDetailViewController

@synthesize searchTypeItem,idItem,latItem,lngItem,mapView,mapInfoSegment,mapTypeSegment,titleMap,subTitleMap,myTextView,myToolbar,scrollLabel;
...
- (void)viewDidLoad
{
    ...
    */
    ...
    if ([searchTypeItem isEqualToString:@"PresubmissionPlans"] || [searchTypeItem isEqualToString:@"PublicHearing"]) {
        [mapTypeSegment removeAllSegments];
        [mapTypeSegment insertSegmentWithTitle:@"Email" atIndex:0 animated:NO];
        [mapTypeSegment insertSegmentWithTitle:@"Call" atIndex:1 animated:NO];
        [mapTypeSegment insertSegmentWithTitle:@"Calendar" atIndex:2 animated:NO];
    } else if ([searchTypeItem isEqualToString:@"SiteDevelopmentPlans"] || [searchTypeItem isEqualToString:@"SubdivisionPlans"] || [searchTypeItem isEqualToString:@"WaiverPetitions"]) {
        [mapTypeSegment removeAllSegments];
        [mapTypeSegment insertSegmentWithTitle:@"Email" atIndex:0 animated:NO];
        [mapTypeSegment insertSegmentWithTitle:@"Call" atIndex:1 animated:NO];
    }
    myTextView.delegate = self;
    [self resizeSegmentsToFitTitles:mapTypeSegment];
    [self start];
}
...
-(void)start {
    ...
    NSLog(@"%@-%f-%f-%@", idItem,latItem,lngItem,searchTypeItem);
    NSString *str;
    
    if ([searchTypeItem isEqualToString:@"SiteDevelopmentPlans"]) {
        str = [NSString stringWithFormat:@"http://data.howardcountymd.gov/iOSsearchPlans/getSDPSIP.asp?id=%@&pType=%@&cenX=%f&cenY=%f", idItem,searchTypeItem,lngItem,latItem];
        self.navigationItem.title = @"Site Development Plans";
    } else if ([searchTypeItem isEqualToString:@"SubdivisionPlans"]) {
        str = [NSString stringWithFormat:@"http://data.howardcountymd.gov/iOSsearchPlans/getSDPSIP.asp?id=%@&pType=%@&cenX=%f&cenY=%f", idItem,searchTypeItem,lngItem,latItem];
        self.navigationItem.title = @"Subdivision Plans";
    } else if ([searchTypeItem isEqualToString:@"PresubmissionPlans"]) {
        str = [NSString stringWithFormat:@"http://data.howardcountymd.gov/iOSsearchPlans/getPresubmission.asp?id=%@&cenX=%f&cenY=%f", idItem,lngItem,latItem];
        self.navigationItem.title = @"Presubmission Community Meetings";
    } else if ([searchTypeItem isEqualToString:@"PublicHearing"]) {
        str = [NSString stringWithFormat:@"http://data.howardcountymd.gov/iOSsearchPlans/getPublicHearing.asp?id=%@&cenX=%f&cenY=%f", idItem,lngItem,latItem];
        self.navigationItem.title = @"Public Hearing";
    } else if ([searchTypeItem isEqualToString:@"WaiverPetitions"]) {
        str = [NSString stringWithFormat:@"http://data.howardcountymd.gov/iOSsearchPlans/getWaiverPetition.asp?id=%@&cenX=%f&cenY=%f", idItem,lngItem,latItem];
        self.navigationItem.title = @"Waiver Petitions";
    }
    
    mailSubject = self.navigationItem.title;
    ...
    for (int i = 0; i < [json count]; i++) {
        ...
        myTextView.text = [info objectForKey:@"information"];
        pPhone = [info objectForKey:@"pPhone"];
        ePhone = [info objectForKey:@"ePhone"];
        pMail = [info objectForKey:@"pMail"];
        eMail = [info objectForKey:@"eMail"];
        fName = [info objectForKey:@"fName"];
        fNumber = [info objectForKey:@"fNumber"];
        if ([searchTypeItem isEqualToString:@"PresubmissionPlans"] || [searchTypeItem isEqualToString:@"PublicHearing"]) {
            startDD = [[info objectForKey:@"startDD"]integerValue];
            startMM = [[info objectForKey:@"startMM"]integerValue];
            startYY = [[info objectForKey:@"startYY"]integerValue];
            startHour = [[info objectForKey:@"startHour"]integerValue];
            startMin = [[info objectForKey:@"startMin"]integerValue];
            NSLog(@"Date=%iMonth=%iYear=%iHour=%iMinute=%i", startDD,startMM,startYY,startHour,startMin);
        } else if ([searchTypeItem isEqualToString:@"SiteDevelopmentPlans"] || [searchTypeItem isEqualToString:@"SubdivisionPlans"] || [searchTypeItem isEqualToString:@"WaiverPetitions"]) {
            myPDF = [info objectForKey:@"PDF"];
            //NSLog(@"PDF=%@", myPDF);
            //NSLog(@"email=%@ and %@", pMail,eMail);
            //NSLog(@"Info=%@", myTextView.text);
            
            if ([myPDF isEqualToString:@"not available"]) {
                self.navigationItem.rightBarButtonItem = nil;
            } else {
                UIBarButtonItem *pdfButton = [[UIBarButtonItem alloc]initWithTitle:@"PDF" style:UIBarButtonItemStylePlain target:self action:@selector(openPDF)];
                [self.navigationItem setRightBarButtonItem:pdfButton];
            }
        }
    }
}
...
- (void)phoneCall {
    if ([pPhone isEqualToString:@"not available"]) {
        if ([ePhone isEqualToString:@"not available"]) {
            actionSht = [[UIActionSheet alloc]initWithTitle:@"Phone number not available!" delegate:self cancelButtonTitle:@"Close" destructiveButtonTitle:nil otherButtonTitles:nil];
        } else {
            actionSht = [[UIActionSheet alloc]initWithTitle:@"Phone" delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"Call County Engineer",nil];
        }
    } else {
        if ([ePhone isEqualToString:@"not available"]) {
            if ([searchTypeItem isEqualToString:@"PresubmissionPlans"]) {
                actionSht = [[UIActionSheet alloc]initWithTitle:@"Phone" delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"Call for Information",nil];
            } else {
                actionSht = [[UIActionSheet alloc]initWithTitle:@"Phone" delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"Call County Planner",nil];
            }
            
        } else {
            actionSht = [[UIActionSheet alloc]initWithTitle:@"Phone" delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"Call County Engineer",@"Call County Planner",nil];
        }
    }
...
- (void)emailMain {
    if ([pMail isEqualToString:@"not available"]) {
        if ([eMail isEqualToString:@"not available"]) {
            actionSht = [[UIActionSheet alloc]initWithTitle:@"Email not available!" delegate:self cancelButtonTitle:@"Close" destructiveButtonTitle:nil otherButtonTitles:nil];
        } else {
            actionSht = [[UIActionSheet alloc]initWithTitle:@"Email" delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"Email County Engineer",nil];
        }
    } else {
        if ([eMail isEqualToString:@"not available"]) {
            actionSht = [[UIActionSheet alloc]initWithTitle:@"Email" delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"Email County Planner",nil];
            
        } else {
            actionSht = [[UIActionSheet alloc]initWithTitle:@"Email" delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"Email County Engineer",@"Email County Planner",nil];
        }
    }
    [actionSht showFromToolbar:(UIToolbar *)myToolbar];
}
...
- (void)openCalendar {
    actionSht = [[UIActionSheet alloc]initWithTitle:[NSString stringWithFormat:@"Add event name '%@' to my iPhone calendar",fName] delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"Add Meeting",nil];
    [actionSht showFromToolbar:(UIToolbar *)myToolbar];
}

-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    NSString *buttonTitle = [actionSheet buttonTitleAtIndex:buttonIndex];
    if ([buttonTitle isEqualToString:@"Call County Engineer"]) {
        [self getCall:(NSString *)ePhone];
    }
    else if ([buttonTitle isEqualToString:@"Call County Planner"]) {
        [self getCall:(NSString *)pPhone];
    }
    else if ([buttonTitle isEqualToString:@"Call for Information"]) {
        [self getCall:(NSString *)pPhone];
    }
    else if ([buttonTitle isEqualToString:@"Email County Engineer"]) {
        [self sendEmail:(NSString *)eMail];
    }
    else if ([buttonTitle isEqualToString:@"Email County Planner"]) {
        [self sendEmail:(NSString *)pMail];
    }
    else if ([buttonTitle isEqualToString:@"Add Meeting"]) {
        [self setCalendar];
    }
    
    [self getMessages];
    [mapTypeSegment setSelectedSegmentIndex:UISegmentedControlNoSegment];
}

-(void)getCall:(NSString *)phoneNum {
    NSString *phoneStr = [NSString stringWithFormat:@"tel:%@",phoneNum];
    if ([phoneStr isEqualToString:@"tel:"]) {
        alertViewer = [[CustomAlertView alloc] initWithTitle:@"Call" message:@"Phone number not available!" delegate:nil cancelButtonTitle:@"Close" otherButtonTitles:nil];
        [alertViewer show];
    } else {
...
- (void)sendEmail:(NSString *)emailAddr {
    NSString *myEmail =[emailAddr stringByReplacingOccurrencesOfString:@"@howardcountymd.gov" withString:@""];
    NSString *myMessage =[myTextView.text stringByReplacingOccurrencesOfString:@"\n" withString:@"<br>"];
    NSString *myMessage1;
    if ([searchTypeItem isEqualToString:@"SiteDevelopmentPlans"] || [searchTypeItem isEqualToString:@"SubdivisionPlans"] || [searchTypeItem isEqualToString:@"WaiverPetitions"]) {
        if ([myPDF isEqualToString:@"not available"]) {
            myMessage1 = [NSString stringWithFormat:@"<p>Dear %@,</p><p>PDF %@</p>%@", myEmail,myPDF,myMessage];
        } else {
            myMessage1 = [NSString stringWithFormat:@"<p>Dear %@,</p><p>Click <a href=%@>PDF</a> to download</p>%@", myEmail,myPDF,myMessage];
        }
    } else if ([searchTypeItem isEqualToString:@"PresubmissionPlans"] || [searchTypeItem isEqualToString:@"PublicHearing"]){
        myMessage1 = [NSString stringWithFormat:@"<p>Dear %@,</p>%@", myEmail,myMessage];
    }
    
    MFMailComposeViewController *mailController = [[MFMailComposeViewController alloc]init];
    [mailController setMailComposeDelegate:self];
    mailController.navigationBar.tintColor = [UIColor blackColor];
    NSString *message = [NSString stringWithFormat:@"%@", myMessage1];
...
-(IBAction)setMap:(id)sender {
    switch(((UISegmentedControl *) sender).selectedSegmentIndex) {
        case 0:
            if ([[mapTypeSegment titleForSegmentAtIndex:0] isEqualToString:@"Road"]) {
                mapView.mapType = MKMapTypeStandard;
            } else if ([[mapTypeSegment titleForSegmentAtIndex:0] isEqualToString:@"Email"]) {
                [self emailMain];
            }
            break;
        case 1:
            if ([[mapTypeSegment titleForSegmentAtIndex:1] isEqualToString:@"Satellite"]) {
                mapView.mapType = MKMapTypeSatellite;
            } else if ([[mapTypeSegment titleForSegmentAtIndex:1] isEqualToString:@"Call"]) {
                [self phoneCall];
            }
            break;
        case 2:
            if ([[mapTypeSegment titleForSegmentAtIndex:2] isEqualToString:@"Hybrid"]) {
                mapView.mapType = MKMapTypeHybrid;
            } else if ([[mapTypeSegment titleForSegmentAtIndex:2] isEqualToString:@"Calendar"]) {
                [self openCalendar];
            }
            break;
            default:
...
- (IBAction)setMapInfo:(id)sender {
    switch(((UISegmentedControl *) sender).selectedSegmentIndex) {
        case 0:
            ...
            [self setPin];
            [mapTypeSegment removeAllSegments];
            [mapTypeSegment insertSegmentWithTitle:@"Road" atIndex:0 animated:NO];
            [mapTypeSegment insertSegmentWithTitle:@"Satellite" atIndex:1 animated:NO];
            [mapTypeSegment insertSegmentWithTitle:@"Hybrid" atIndex:2 animated:NO];
            [self resizeSegmentsToFitTitles:mapTypeSegment];
            break;
        case 1:
            mapView.hidden = YES;
...
- (void)setPin {
    ...
    ann.title = self.titleMap;
    ann.subtitle = self.subTitleMap;
...
-(MKAnnotationView *)mapView:(MKMapView *)mView viewForAnnotation:(id <MKAnnotation>)annotation
{
    ...
    if ([annotation isKindOfClass:[Annotation class]])
    {
        ...
        if (!pinView)
        {
            ...
            imgname = [NSString stringWithFormat:@"%@.png", searchTypeItem];
            ...
            CGSize maxSize = CGRectInset(self.view.bounds,
                                         [SearchPlanDetailViewController annotationPadding],
                                         [SearchPlanDetailViewController annotationPadding]).size;
            maxSize.height -= self.navigationController.navigationBar.frame.size.height + [SearchPlanDetailViewController calloutHeight];
        ...
        [eventStore requestAccessToEntityType:EKEntityTypeEvent completion:^(BOOL granted, NSError *error) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                else
                {
                    ...
                    event.title = [NSString stringWithFormat:@"%@: %@", fName,fNumber];
