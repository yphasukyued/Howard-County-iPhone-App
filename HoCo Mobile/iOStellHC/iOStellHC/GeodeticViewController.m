...
@implementation GeodeticViewController

@synthesize latItem,lngItem,locationManager,geodeticTableView,mapView,mapTypeSegment,mapInfoSegment,scrollLabel;
...
{
    [super viewDidLoad];
    self.geodeticTableView.delegate = self;
    self.geodeticTableView.dataSource = self;
    ...
    //UIBarButtonItem *searchButton = [[UIBarButtonItem alloc]initWithTitle:@"Search" style:UIBarButtonItemStylePlain target:self action:@selector(showMenu)];
    //[self.navigationItem setRightBarButtonItem:searchButton];
    
    UIBarButtonItem *searchButton = [[UIBarButtonItem alloc]initWithBarButtonSystemItem:UIBarButtonSystemItemSearch target:self action:@selector(searchGeodetic)];
                                    
    //UIBarButtonItem *searchButton = [[UIBarButtonItem alloc]initWithTitle:@"Search" style:UIBarButtonItemStylePlain target:self action:@selector(searchGeodetic)];
    [self.navigationItem setRightBarButtonItem:searchButton];
    ...
    alertViewer = [[CustomAlertView alloc] initWithTitle:@"What's next" message:@"Tap ‘Search’ button to search from your current location (1 mile radius).  Touch and drag on map to change the search location." delegate:nil cancelButtonTitle:@"Close" otherButtonTitles:nil];
...
-(void) searchGeodetic {
    ...
    NSString *str1 = [NSString stringWithFormat:@"http://data.howardcountymd.gov/iOSgeodetic/getGeodeticList.asp?cenY=%f&cenX=%f", latItem,lngItem];
    ...
    if (json.count==0) {
        alertViewer = [[CustomAlertView alloc] initWithTitle:@"Not Found" message:@"No Geodetic Control in 1 miles radius of map center, please make sure that you are in Howard County Maryland" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"Search Again",nil];
        [alertViewer setDelegate:self];
        [alertViewer show];
    } else {
        [geodeticTableView reloadData];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.imageView.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@.png", [info objectForKey:@"stype"]]];
    cell.textLabel.text = [info objectForKey:@"TYPE"];
    cell.detailTextLabel.text = [info objectForKey:@"NO"];
    return cell;
...
{
    GeodeticDetailViewController *detailVC = [[GeodeticDetailViewController alloc]init];
    NSDictionary *info = [json objectAtIndex:indexPath.row];
    detailVC.searchTypeItem = [info objectForKey:@"stype"];
    detailVC.fnumberItem = [info objectForKey:@"NO"];
...
- (IBAction)setMapInfo:(id)sender {
    switch(((UISegmentedControl *) sender).selectedSegmentIndex) {
        case 0:
            ...
            geodeticTableView.hidden = YES;
            break;
        case 1:
            mapView.hidden = YES;
            mapTypeSegment.hidden = YES;
            geodeticTableView.hidden = NO;
            break;
        default:
            break;
    }
...
- (void)setPin {
    ...
    circle = [MKCircle circleWithCenterCoordinate:centerCoordinate radius:1609.344];
    ...
    for (int i = 0; i < [json count]; i++) {
            ...
            if ([[loc objectForKey:@"stype"] isEqualToString:@"GeodeticStations"]) {
                StationsAnnotation *ann = [[StationsAnnotation alloc] init];
                ann.title = [loc objectForKey:@"TYPE"];
                ann.subtitle = [loc objectForKey:@"NO"];
                ann.coordinate = location;
                [mapView addAnnotation:ann];
            } else if ([[loc objectForKey:@"stype"] isEqualToString:@"GeodeticBenchmarks"]) {
                BenchmarksAnnotation *ann = [[BenchmarksAnnotation alloc] init];
                ann.title = [loc objectForKey:@"TYPE"];
                ann.subtitle = [loc objectForKey:@"NO"];
                ann.coordinate = location;
                [mapView addAnnotation:ann];
...
-(MKAnnotationView *)mapView:(MKMapView *)mView viewForAnnotation:(id <MKAnnotation>)annotation
{
    ...
    if ([annotation isKindOfClass:[StationsAnnotation class]]) {
        ...
        if (!pinView)
        {
            ...
            CGSize maxSize = CGRectInset(self.view.bounds,
                                         [GeodeticViewController annotationPadding],
                                         [GeodeticViewController annotationPadding]).size;
            maxSize.height -= self.navigationController.navigationBar.frame.size.height + [GeodeticViewController calloutHeight];
    ...
    else if ([annotation isKindOfClass:[BenchmarksAnnotation class]]) {
        // try to dequeue an existing pin view first
        ...
        MKAnnotationView *pinView = (MKAnnotationView *) [mapView dequeueReusableAnnotationViewWithIdentifier:annotationIdentifier1];
            if (!pinView)
            {
                ...
                imgname = @"GeodeticBenchmarks.png";
                ...
                [rightButton addTarget:self
                                action:@selector(showDetails:[NSString stringWithFormat:@"%@",])
                      forControlEvents:UIControlEventTouchUpInside];
                annotationView1.rightCalloutAccessoryView = rightButton;
                */
...
- (void)mapView:(MKMapView *)mv annotationView:(MKAnnotationView *)pin calloutAccessoryControlTapped:(UIControl *)control {
    ...
    if ([control isKindOfClass:[UIButton class]]) {
        ...
        GeodeticDetailViewController *detailVC = [[GeodeticDetailViewController alloc]init];
        detailVC.searchTypeItem = theAnnotation.title;
        detailVC.fnumberItem = theAnnotation.subtitle;
